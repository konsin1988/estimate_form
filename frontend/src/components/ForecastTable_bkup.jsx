...
export default function ForecastTable({ target_user, target_login, init_frc, list_frc, is_admin }) {
    const dispatch = useDispatch();
    const { data, loading, error } = useSelector((state) => state.profile);
    
    const [colsVisible, setColsVisible] = useState(0);
    function handleColsVisible(cols) {
				setColsVisible(cols)
		}
...
// <SubmitButton user={user} login={login} frc={frc} />
